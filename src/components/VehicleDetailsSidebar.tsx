...
import { ArrowLeft, Car, Calendar, Wrench, FileText, QrCode, Share2, MapPin, Clock, DollarSign, Plus, Image, AlertTriangle, CheckCircle, Search, Menu, X } from 'lucide-react';
...
const VehicleDetailsSidebar: React.FC<VehicleDetailsSidebarProps> = ({ 
  vehicleId, 
  onBack, 
  showBackButton = true,
  onVehicleChange 
  ...
  const handleVehicleSelect = (selectedVehicleId: string) => {
    if (onVehicleChange) {
      onVehicleChange(selectedVehicleId);
    }
  ...
  const filteredVehicles = allVehicles.filter(v => 
    v && v.plateNumber && v.plateNumber.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v && v.model && v.model.toLowerCase().includes(searchTerm.toLowerCase()) ||
    v && v.barcode && v.barcode.toLowerCase().includes(searchTerm.toLowerCase())
  );
        ...
        <div className="flex flex-col h-full">
          ...
          <div className="p-4 border-b bg-gradient-to-r from-blue-600 to-blue-700 text-white">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold">רכבים במערכת</h2>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="mt-3 relative">
              <Search className="absolute right-3 top-1/2 transform -translate-y-1/2 text-blue-200 w-4 h-4" />
              <Input
                placeholder="חיפוש רכב..."
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="pr-10 bg-blue-50 border-blue-200 text-gray-900 placeholder:text-blue-400"
              />
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="font-semibold text-gray-900">{v.plateNumber}</span>
                      </div>
                      <p className="text-sm text-gray-600 mb-1">{v.model}</p>
                      <div className="flex items-center gap-2">
                        <Badge className={`text-xs ${getStatusColor(v.maintenanceStatus)}`}>
                          {v.maintenanceStatus}
                        </Badge>
                        <span className="text-xs text-gray-500">{v.barcode}</span>
                      </div>
                    </div>
                    {v.id === vehicleId && (
            ...
            {filteredVehicles.length === 0 && (
              <div className="p-8 text-center">
                ...
                <p className="text-gray-600">לא נמצאו רכבים</p>
              </div>
            )}
          </div>
        </div>
